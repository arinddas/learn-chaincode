...
func (t *NumberPortabilityChaincode) EligibilityConfirmQuery(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if(args[1] != "user_type1_0") {
	return nil, fmt.Errorf("User: %s not Authorized for this transaction:  ", string(args[1]))
	}
    ...
    if len(row.Columns) != 0{
		...
		EmailId := row.Columns[3].GetString_()
		PortabilityIndicator := row.Columns[4].GetString_()
		
		str := `{"Number": "` + args[0] + `", "ServiceProvider": "` + ServiceProvider + `", "CustomerName": "` + CustomerName + `", "EmailId": "` + EmailId + `", "PortabilityIndicator": "` + PortabilityIndicator + `"}`
